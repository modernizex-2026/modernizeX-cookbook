...
const INPUTS = ['WK-KEY-CUST'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-KEY-CUST': 'WK-KEY-CUST',
};
const NUMERIC = new Set<string>(['WK-KEY-CUST']);
        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Customer Code</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-KEY-CUST')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-KEY-CUST')" :maxlength="activeWidth ?? 6" data-active="true" :data-field-name="'WK-KEY-CUST'" @input="e => onInput(e, 'WK-KEY-CUST')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-KEY-CUST') }}</span>
            </div>
        </div>
      </div>
    </template>
