...
const INPUTS = ['WK-EDIT-LN', 'WK-EDIT-QTY'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-EDIT-LN': 'WK-EDIT-LN',
  'WK-EDIT-QTY': 'WK-EDIT-QTY',
};
const NUMERIC = new Set<string>(['WK-EDIT-LN', 'WK-EDIT-QTY']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-EDIT-LN': valueOfCache('WK-EDIT-LN'), 'WK-EDIT-QTY': valueOfCache('WK-EDIT-QTY') }),
    pfOverrides: { F3: '03', F4: '04', F6: '06' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Chg Ln</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-EDIT-LN')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-EDIT-LN')" :maxlength="activeWidth ?? 3" data-active="true" :data-field-name="'WK-EDIT-LN'" @input="e => onInput(e, 'WK-EDIT-LN')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-EDIT-LN') }}</span>
              <span class="scr-label" style="font-size:13px; white-space:nowrap; margin-left:8px;">Ship</span>
              <input v-if="isFieldActive('WK-EDIT-QTY')" class="scr-input scr-numeric" style="width:11ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-EDIT-QTY')" :maxlength="activeWidth ?? 9" data-active="true" :data-field-name="'WK-EDIT-QTY'" @input="e => onInput(e, 'WK-EDIT-QTY')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:11ch; max-width:100%; display:inline-block;">{{ valueOf('WK-EDIT-QTY') }}</span>
              <p class="scr-hint" style="flex-basis:100%; margin:0;">ENTER=set PF3=done</p>
              <p class="scr-hint" style="flex-basis:100%; margin:0;">Ship: ENTER=set PF3=done</p>
            </div>
        </div>
      </div>
    </template>
