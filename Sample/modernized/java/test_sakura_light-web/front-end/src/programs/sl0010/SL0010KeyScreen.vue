...
const INPUTS = ['WK-SEL-SHIP', 'WK-SEL-CUST'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-SEL-SHIP': 'WK-SEL-SHIP',
  'WK-SEL-CUST': 'WK-SEL-CUST',
};
const NUMERIC = new Set<string>(['WK-SEL-SHIP', 'WK-SEL-CUST']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-SEL-SHIP': valueOfCache('WK-SEL-SHIP'), 'WK-SEL-CUST': valueOfCache('WK-SEL-CUST') }),
    pfOverrides: { F3: '03' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Shipment No</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SEL-SHIP')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SEL-SHIP')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-SEL-SHIP'" @input="e => onInput(e, 'WK-SEL-SHIP')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SEL-SHIP') }}</span>
            </div>
          <div style="grid-column: 1 / -1; text-align:center; color:#8a94a6; font-size:12px; margin:2px 0;">— or key a direct sale —</div>
