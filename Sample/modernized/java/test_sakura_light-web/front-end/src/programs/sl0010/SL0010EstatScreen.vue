    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr; margin-bottom:8px;">
          ...
          <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; min-width:0;"><span class="scr-value" style="width:9ch; max-width:100%; display:inline-block;">{{ valueOf('WK-DCNT') }}</span> <span class="scr-label" style="white-space:nowrap; font-size:13px; padding-top:9px;">Net Total</span> <span class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-NET-TOTAL') }}</span></div>
        </div>
      </div>
    </template>
    <template #footer>
