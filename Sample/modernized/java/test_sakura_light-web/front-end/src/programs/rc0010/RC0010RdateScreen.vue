...
const INPUTS = ['RH-DATE'];
const FIELD_DATA_MAP: Record<string, string> = {
  'RH-DATE': 'RH-DATE',
};
const NUMERIC = new Set<string>(['RH-DATE']);
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Recv Date</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('RH-DATE')" class="scr-input scr-numeric" style="width:10ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('RH-DATE')" :maxlength="activeWidth ?? 8" data-active="true" :data-field-name="'RH-DATE'" @input="e => onInput(e, 'RH-DATE')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('RH-DATE') }}</span>
            </div>
        </div>
      </div>
    </template>
