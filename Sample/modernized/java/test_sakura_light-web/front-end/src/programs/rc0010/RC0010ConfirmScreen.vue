    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Lines to receive</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-RECV-CNT') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Post receiving ? (Y/N)</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-CONFIRM')" class="scr-input" style="width:8ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('WK-CONFIRM')" :maxlength="activeWidth ?? 1" data-active="true" :data-field-name="'WK-CONFIRM'" @input="e => onInput(e, 'WK-CONFIRM')" @keydown="onKey" /><span v-else class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-CONFIRM') }}</span>
            </div>
        </div>
      </div>
    </template>
