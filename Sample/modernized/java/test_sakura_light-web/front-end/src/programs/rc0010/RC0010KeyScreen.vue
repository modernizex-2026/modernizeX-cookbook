...
const INPUTS = ['WK-PO-KEY'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-PO-KEY': 'WK-PO-KEY',
};
const NUMERIC = new Set<string>(['WK-PO-KEY']);
        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">PO Number</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-PO-KEY')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-PO-KEY')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-PO-KEY'" @input="e => onInput(e, 'WK-PO-KEY')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-PO-KEY') }}</span>
            </div>
        </div>
      </div>
    </template>
