...
const INPUTS = ['CP-CUST', 'CP-PROD'];
const FIELD_DATA_MAP: Record<string, string> = {
  'CP-CUST': 'CP-CUST',
  'CP-PROD': 'CP-PROD',
};
const NUMERIC = new Set<string>(['CP-CUST', 'CP-PROD']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'CP-CUST': valueOfCache('CP-CUST'), 'CP-PROD': valueOfCache('CP-PROD') }),
    pfOverrides: { F3: '03' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Customer Code</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('CP-CUST')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('CP-CUST')" :maxlength="activeWidth ?? 6" data-active="true" :data-field-name="'CP-CUST'" @input="e => onInput(e, 'CP-CUST')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('CP-CUST') }}</span>
            </div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Product Code</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('CP-PROD')" class="scr-input scr-numeric" style="width:10ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('CP-PROD')" :maxlength="activeWidth ?? 8" data-active="true" :data-field-name="'CP-PROD'" @input="e => onInput(e, 'CP-PROD')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('CP-PROD') }}</span>
            </div>
        </div>
      </div>
    </template>
