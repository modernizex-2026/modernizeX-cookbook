...
const INPUTS = ['WK-D-QTY', 'WK-D-PRICE'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-D-QTY': 'WK-D-QTY',
  'WK-D-PRICE': 'WK-D-PRICE',
};
const NUMERIC = new Set<string>(['WK-D-QTY', 'WK-D-PRICE']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-D-QTY': valueOfCache('WK-D-QTY'), 'WK-D-PRICE': valueOfCache('WK-D-PRICE') }),
    pfOverrides: { F3: '03' },
...
<template>
  <Modal :visible="true" title="Change detail line" min-width="510px" max-width="90vw" :close-able="false">
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Line</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-CMD-ARG') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Product</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:9ch; max-width:100%; display:inline-block;">{{ valueOf('WK-D-PROD') }}</span> <span class="scr-desc-box" style="width:40ch; max-width:100%; margin-left:8px;">{{ valueOf('WK-D-NAME') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Warehouse</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-D-WHSE') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Quantity</label>
