...
const INPUTS = ['WK-SEL-ORD'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-SEL-ORD': 'WK-SEL-ORD',
};
const NUMERIC = new Set<string>(['WK-SEL-ORD']);
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Order Number</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SEL-ORD')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SEL-ORD')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-SEL-ORD'" @input="e => onInput(e, 'WK-SEL-ORD')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SEL-ORD') }}</span>
            </div>
        </div>
      </div>
    </template>
