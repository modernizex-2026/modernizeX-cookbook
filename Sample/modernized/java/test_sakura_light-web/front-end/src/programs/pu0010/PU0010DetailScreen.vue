...
const INPUTS = ['WK-D-PROD', 'WK-D-WHSE', 'WK-D-QTY', 'WK-D-COST'];
...
const NUMERIC = new Set<string>(['WK-D-PROD', 'WK-D-WHSE', 'WK-D-QTY', 'WK-D-COST']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-D-PROD': valueOfCache('WK-D-PROD'), 'WK-D-WHSE': valueOfCache('WK-D-WHSE'), 'WK-D-QTY': valueOfCache('WK-D-QTY'), 'WK-D-COST': valueOfCache('WK-D-COST') }),
...
<template>
  <Modal :visible="true" title="Detail line entry" min-width="510px" max-width="90vw" :close-able="false">
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          ...
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Unit Cost</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-D-COST')" class="scr-input scr-numeric" style="width:11ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-D-COST')" :maxlength="activeWidth ?? 9" data-active="true" :data-field-name="'WK-D-COST'" @input="e => onInput(e, 'WK-D-COST')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:11ch; max-width:100%; display:inline-block;">{{ valueOf('WK-D-COST') }}</span>
            </div>
        </div>
      </div>
    </template>
