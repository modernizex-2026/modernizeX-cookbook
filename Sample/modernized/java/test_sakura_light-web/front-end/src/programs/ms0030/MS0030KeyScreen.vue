        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Product Code</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('SC-KEY')" class="scr-input scr-numeric" style="width:10ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('SC-KEY')" :maxlength="activeWidth ?? 8" data-active="true" :data-field-name="'SC-KEY'" @input="e => onInput(e, 'SC-KEY')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('SC-KEY') }}</span>
            </div>
        </div>
      </div>
    </template>
