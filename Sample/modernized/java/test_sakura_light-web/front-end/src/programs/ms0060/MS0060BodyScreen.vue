...
const INPUTS = ['CT-NAME', 'CT-PARENT', 'CT-LEVEL'];
const FIELD_DATA_MAP: Record<string, string> = {
  'CT-NAME': 'CT-NAME',
  'CT-PARENT': 'CT-PARENT',
  'CT-LEVEL': 'CT-LEVEL',
};
const NUMERIC = new Set<string>(['CT-PARENT', 'CT-LEVEL']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'CT-NAME': valueOfCache('CT-NAME'), 'CT-PARENT': valueOfCache('CT-PARENT'), 'CT-LEVEL': valueOfCache('CT-LEVEL') }),
        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Name</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('CT-NAME')" class="scr-input" style="width:32ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('CT-NAME')" :maxlength="activeWidth ?? 30" data-active="true" :data-field-name="'CT-NAME'" @input="e => onInput(e, 'CT-NAME')" @keydown="onKey" /><span v-else class="scr-value" style="width:32ch; max-width:100%; display:inline-block;">{{ valueOf('CT-NAME') }}</span>
            </div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Parent Code</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('CT-PARENT')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('CT-PARENT')" :maxlength="activeWidth ?? 4" data-active="true" :data-field-name="'CT-PARENT'" @input="e => onInput(e, 'CT-PARENT')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('CT-PARENT') }}</span>
              <span class="scr-desc-box" style="width:30ch; max-width:100%; margin-left:8px;">{{ valueOf('WK-PARENT-NAME') }}</span>
            </div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Level</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('CT-LEVEL')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('CT-LEVEL')" :maxlength="activeWidth ?? 1" data-active="true" :data-field-name="'CT-LEVEL'" @input="e => onInput(e, 'CT-LEVEL')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('CT-LEVEL') }}</span>
              <p class="scr-hint" style="flex-basis:100%; margin:0;">0=root  1-9 depth</p>
            </div>
        </div>
      </div>
    </template>
