...
const INPUTS = ['WK-PICK-LINE', 'WK-PICK-QTY'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-PICK-LINE': 'WK-PICK-LINE',
  'WK-PICK-QTY': 'WK-PICK-QTY',
};
const NUMERIC = new Set<string>(['WK-PICK-LINE', 'WK-PICK-QTY']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-PICK-LINE': valueOfCache('WK-PICK-LINE'), 'WK-PICK-QTY': valueOfCache('WK-PICK-QTY') }),
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Line</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-PICK-LINE')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-PICK-LINE')" :maxlength="activeWidth ?? 3" data-active="true" :data-field-name="'WK-PICK-LINE'" @input="e => onInput(e, 'WK-PICK-LINE')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-PICK-LINE') }}</span>
              <span class="scr-label" style="font-size:13px; white-space:nowrap; margin-left:8px;">Credit Qty</span>
              <input v-if="isFieldActive('WK-PICK-QTY')" class="scr-input scr-numeric" style="width:11ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-PICK-QTY')" :maxlength="activeWidth ?? 9" data-active="true" :data-field-name="'WK-PICK-QTY'" @input="e => onInput(e, 'WK-PICK-QTY')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:11ch; max-width:100%; display:inline-block;">{{ valueOf('WK-PICK-QTY') }}</span>
              <p class="scr-hint" style="flex-basis:100%; margin:0;">Credit Qty:</p>
            </div>
        </div>
      </div>
    </template>
