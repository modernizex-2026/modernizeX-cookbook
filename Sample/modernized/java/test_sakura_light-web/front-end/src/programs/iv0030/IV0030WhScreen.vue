...
const INPUTS = ['WK-WHSE-IN'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-WHSE-IN': 'WK-WHSE-IN',
};
const NUMERIC = new Set<string>(['WK-WHSE-IN']);
        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Warehouse</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-WHSE-IN')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-WHSE-IN')" :maxlength="activeWidth ?? 3" data-active="true" :data-field-name="'WK-WHSE-IN'" @input="e => onInput(e, 'WK-WHSE-IN')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-WHSE-IN') }}</span>
              <span class="scr-desc-box" style="width:30ch; max-width:100%; margin-left:8px;">{{ valueOf('WK-WH-NAME') }}</span>
            </div>
        </div>
      </div>
    </template>
