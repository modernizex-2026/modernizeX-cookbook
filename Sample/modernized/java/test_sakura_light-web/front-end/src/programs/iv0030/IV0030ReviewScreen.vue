...
const { valueOf, submit } = useScreenForm(props, { pfOverrides: { F3: '03', F4: '04' }, typed });
...
const PACKED_COLS: { label: string; start: number }[] = [{ label: 'Prod', start: 0 }, { label: 'Name', start: 8 }, { label: 'Book', start: 24 }, { label: 'Count', start: 34 }, { label: 'Diff', start: 44 }];
    ...
    <div class="scr-page-body">
      <div class="scr-form-section" style="box-sizing:border-box; padding:16px 20px 14px;">
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr; margin-bottom:8px;">
          ...
          <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; min-width:0;"><span class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-WHSE-IN') }}</span> <span class="scr-value" style="width:32ch; max-width:100%; display:inline-block;">{{ valueOf('WK-WH-NAME') }}</span></div>
        </div>
        ...
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr; margin-bottom:8px;">
          ...
          <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; min-width:0;"><span class="scr-value" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('WK-RPAGE') }}</span> <span class="scr-label" style="white-space:nowrap; font-size:13px; padding-top:9px;">of items</span> <span class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-TCNT') }}</span></div>
        </div>
      </div>
    </div>
