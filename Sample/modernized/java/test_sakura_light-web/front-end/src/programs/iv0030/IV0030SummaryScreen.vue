    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Items counted</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-TCNT') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">With diff</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-DCNT') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Post count (Y/N)</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('SC-CONF')" class="scr-input" style="width:8ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('SC-CONF')" :maxlength="activeWidth ?? 1" data-active="true" :data-field-name="'SC-CONF'" @input="e => onInput(e, 'SC-CONF')" @keydown="onKey" /><span v-else class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('SC-CONF') }}</span>
            </div>
        </div>
      </div>
    </template>
