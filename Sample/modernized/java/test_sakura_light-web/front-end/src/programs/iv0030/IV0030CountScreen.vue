...
const INPUTS = ['WK-COUNTED'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-COUNTED': 'WK-COUNTED',
};
const NUMERIC = new Set<string>(['WK-COUNTED']);
      ...
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Warehouse</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:9ch; max-width:100%; display:inline-block;">{{ valueOf('WK-WHSE-IN') }}</span> <span class="scr-desc-box" style="width:30ch; max-width:100%; margin-left:8px;">{{ valueOf('WK-WH-NAME') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Item No</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SHOW-NO') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Product</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:9ch; max-width:100%; display:inline-block;">{{ valueOf('SK-PROD') }}</span> <span class="scr-desc-box" style="width:40ch; max-width:100%; margin-left:8px;">{{ valueOf('WK-PR-NAME') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Book OnHand</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;"><span class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-BOOK') }}</span></div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Counted Qty</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-COUNTED')" class="scr-input scr-numeric" style="width:11ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-COUNTED')" :maxlength="activeWidth ?? 9" data-active="true" :data-field-name="'WK-COUNTED'" @input="e => onInput(e, 'WK-COUNTED')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:11ch; max-width:100%; display:inline-block;">{{ valueOf('WK-COUNTED') }}</span>
            </div>
        </div>
      </div>
    </template>
