...
const INPUTS = ['WK-RECV-KEY'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-RECV-KEY': 'WK-RECV-KEY',
};
const NUMERIC = new Set<string>(['WK-RECV-KEY']);
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Receiving No</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-RECV-KEY')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-RECV-KEY')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-RECV-KEY'" @input="e => onInput(e, 'WK-RECV-KEY')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-RECV-KEY') }}</span>
            </div>
        </div>
      </div>
    </template>
