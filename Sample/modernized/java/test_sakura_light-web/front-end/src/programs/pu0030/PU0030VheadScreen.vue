...
const INPUTS = ['VH-DATE', 'VH-TAX-TYPE'];
const FIELD_DATA_MAP: Record<string, string> = {
  'VH-DATE': 'VH-DATE',
  'VH-TAX-TYPE': 'VH-TAX-TYPE',
};
const NUMERIC = new Set<string>(['VH-DATE', 'VH-TAX-TYPE']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'VH-DATE': valueOfCache('VH-DATE'), 'VH-TAX-TYPE': valueOfCache('VH-TAX-TYPE') }),
    pfOverrides: { F3: '03' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Purch Date</label>
