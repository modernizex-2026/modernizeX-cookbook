...
const INPUTS = ['WK-SEL-NO', 'WK-SEL-CUST', 'WK-SEL-DATE'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-SEL-NO': 'WK-SEL-NO',
  'WK-SEL-CUST': 'WK-SEL-CUST',
  'WK-SEL-DATE': 'WK-SEL-DATE',
};
const NUMERIC = new Set<string>(['WK-SEL-NO', 'WK-SEL-CUST', 'WK-SEL-DATE']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-SEL-NO': valueOfCache('WK-SEL-NO'), 'WK-SEL-CUST': valueOfCache('WK-SEL-CUST'), 'WK-SEL-DATE': valueOfCache('WK-SEL-DATE') }),
    pfOverrides: { F3: '03' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Invoice No</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SEL-NO')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SEL-NO')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-SEL-NO'" @input="e => onInput(e, 'WK-SEL-NO')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SEL-NO') }}</span>
            </div>
          <div style="grid-column: 1 / -1; text-align:center; color:#8a94a6; font-size:12px; margin:2px 0;">— or —</div>
          ...
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">From Date</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SEL-DATE')" class="scr-input scr-numeric" style="width:10ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SEL-DATE')" :maxlength="activeWidth ?? 8" data-active="true" :data-field-name="'WK-SEL-DATE'" @input="e => onInput(e, 'WK-SEL-DATE')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SEL-DATE') }}</span>
            </div>
        </div>
      </div>
    </template>
