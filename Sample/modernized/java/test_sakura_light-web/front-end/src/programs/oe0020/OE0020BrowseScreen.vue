    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">PFkey</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-DUMMY')" class="scr-input" style="width:8ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('WK-DUMMY')" :maxlength="activeWidth ?? 1" data-active="true" :data-field-name="'WK-DUMMY'" @input="e => onInput(e, 'WK-DUMMY')" @keydown="onKey" /><span v-else class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-DUMMY') }}</span>
            </div>
        </div>
      </div>
    </template>
