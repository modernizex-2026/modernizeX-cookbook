...
const INPUTS = ['WK-SRCH-NO', 'WK-SRCH-SUPP', 'WK-SRCH-DATE'];
const FIELD_DATA_MAP: Record<string, string> = {
  'WK-SRCH-NO': 'WK-SRCH-NO',
  'WK-SRCH-SUPP': 'WK-SRCH-SUPP',
  'WK-SRCH-DATE': 'WK-SRCH-DATE',
};
const NUMERIC = new Set<string>(['WK-SRCH-NO', 'WK-SRCH-SUPP', 'WK-SRCH-DATE']);
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'WK-SRCH-NO': valueOfCache('WK-SRCH-NO'), 'WK-SRCH-SUPP': valueOfCache('WK-SRCH-SUPP'), 'WK-SRCH-DATE': valueOfCache('WK-SRCH-DATE') }),
    pfOverrides: { F3: '03' },
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">PO Number</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SRCH-NO')" class="scr-input scr-numeric" style="width:12ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SRCH-NO')" :maxlength="activeWidth ?? 10" data-active="true" :data-field-name="'WK-SRCH-NO'" @input="e => onInput(e, 'WK-SRCH-NO')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:12ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SRCH-NO') }}</span>
            </div>
          <div style="grid-column: 1 / -1; text-align:center; color:#8a94a6; font-size:12px; margin:2px 0;">— or search by supplier —</div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Supplier</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SRCH-SUPP')" class="scr-input scr-numeric" style="width:8ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SRCH-SUPP')" :maxlength="activeWidth ?? 6" data-active="true" :data-field-name="'WK-SRCH-SUPP'" @input="e => onInput(e, 'WK-SRCH-SUPP')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SRCH-SUPP') }}</span>
            </div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">From Date</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-SRCH-DATE')" class="scr-input scr-numeric" style="width:10ch; max-width:100%;" inputmode="numeric" :ref="bindActiveInput" :value="valueOf('WK-SRCH-DATE')" :maxlength="activeWidth ?? 8" data-active="true" :data-field-name="'WK-SRCH-DATE'" @input="e => onInput(e, 'WK-SRCH-DATE')" @keydown="onKey" /><span v-else class="scr-value scr-numeric" style="width:10ch; max-width:100%; display:inline-block;">{{ valueOf('WK-SRCH-DATE') }}</span>
            </div>
        </div>
      </div>
    </template>
