    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">PF6=Next  PF3=Back</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('WK-NAV')" class="scr-input" style="width:8ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('WK-NAV')" :maxlength="activeWidth ?? 1" data-active="true" :data-field-name="'WK-NAV'" @input="e => onInput(e, 'WK-NAV')" @keydown="onKey" /><span v-else class="scr-value" style="width:8ch; max-width:100%; display:inline-block;">{{ valueOf('WK-NAV') }}</span>
            </div>
        </div>
      </div>
    </template>
