    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Region Name</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('RG-NAME')" class="scr-input" style="width:32ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('RG-NAME')" :maxlength="activeWidth ?? 30" data-active="true" :data-field-name="'RG-NAME'" @input="e => onInput(e, 'RG-NAME')" @keydown="onKey" /><span v-else class="scr-value" style="width:32ch; max-width:100%; display:inline-block;">{{ valueOf('RG-NAME') }}</span>
            </div>
        </div>
      </div>
    </template>
