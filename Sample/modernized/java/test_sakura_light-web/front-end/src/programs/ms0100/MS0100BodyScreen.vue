...
const INPUTS = ['BK-NAME', 'BK-BRANCH'];
...
const { isFieldActive, activeWidth, valueOf, submit, onKey, onInput, bindActiveInput } =
  useScreenForm(props, {
    ...
    buildScreenValues: () => ({ 'BK-NAME': valueOfCache('BK-NAME'), 'BK-BRANCH': valueOfCache('BK-BRANCH') }),
    ...
    <template #body>
      <div>
        <div class="scr-form-grid" style="align-items:start; grid-template-columns:100px 1fr;">
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Bank Name</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('BK-NAME')" class="scr-input" style="width:32ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('BK-NAME')" :maxlength="activeWidth ?? 30" data-active="true" :data-field-name="'BK-NAME'" @input="e => onInput(e, 'BK-NAME')" @keydown="onKey" /><span v-else class="scr-value" style="width:32ch; max-width:100%; display:inline-block;">{{ valueOf('BK-NAME') }}</span>
            </div>
          <label class="scr-label" style="text-align:left; padding-top:9px; font-size:13px;">Branch Name</label>
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              <input v-if="isFieldActive('BK-BRANCH')" class="scr-input" style="width:32ch; max-width:100%;" :ref="bindActiveInput" :value="valueOf('BK-BRANCH')" :maxlength="activeWidth ?? 30" data-active="true" :data-field-name="'BK-BRANCH'" @input="e => onInput(e, 'BK-BRANCH')" @keydown="onKey" /><span v-else class="scr-value" style="width:32ch; max-width:100%; display:inline-block;">{{ valueOf('BK-BRANCH') }}</span>
            </div>
        </div>
      </div>
    </template>
