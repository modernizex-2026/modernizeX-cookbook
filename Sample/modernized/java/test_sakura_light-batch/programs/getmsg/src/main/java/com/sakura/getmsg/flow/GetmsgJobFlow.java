package com.sakura.getmsg.flow;

import com.sakura.getmsg.service.GetmsgService;
...
/**
 * Spring Batch wiring for COBOL program GETMSG: the job, its step, the tasklet that runs the
 * program service, and the job/step monitors — assembled in one configuration.
 */
@Configuration
public class GetmsgJobFlow {

    @Bean
    public Job getmsgJob(
            JobRepository jobRepository, @Qualifier("getmsgStep") Step step, JobMonitor listener) {
        ...
        return new JobBuilder("GETMSGJob", jobRepository)
    ...
    public Step getmsgStep(
        ...
        return new StepBuilder("GETMSGStep", jobRepository)
    ...
    @Bean
    public Task getmsgTasklet(ObjectProvider<GetmsgService> serviceProvider) {
        return new Task(serviceProvider);
    }

    @Bean
    public JobMonitor getmsgJobListener() {
        return new JobMonitor();
    }

    @Bean
    public StepMonitor getmsgStepListener() {
    ...
    static class Task implements Tasklet {
        private final ObjectProvider<GetmsgService> serviceProvider;

        Task(ObjectProvider<GetmsgService> serviceProvider) {
        ...
        public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext)
                throws Exception {
            GetmsgService service = serviceProvider.getObject();
            service.execute();
            chunkContext
                    .getStepContext()
                    .getStepExecution()
    ...
    static class JobMonitor implements JobExecutionListener {
        ...
        private static final String JOB_NAME = "GETMSG";
    ...
    static class StepMonitor implements StepExecutionListener {
        ...
        public void beforeStep(StepExecution stepExecution) {
            log.info("[GETMSG] Step starting: {}", stepExecution.getStepName());
        }
        ...
        public ExitStatus afterStep(StepExecution stepExecution) {
            ...
            if (status.isUnsuccessful()) {
                log.error(
                        "[GETMSG] Step FAILED: {} status={} completionCode={}",
                        stepName,
                        status,
                        completionCode);
                stepExecution
                        .getFailureExceptions()
                        .forEach(ex -> log.error("[GETMSG] Exception: {}", ex.getMessage(), ex));
                return new ExitStatus("FAILED")
                        .addExitDescription("completionCode=" + completionCode);
            }

            log.info(
                    "[GETMSG] Step completed: {} status={} completionCode={}",
                    stepName,
                    status,
                    completionCode);
