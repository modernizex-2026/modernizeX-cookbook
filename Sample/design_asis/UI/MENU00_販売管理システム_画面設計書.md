...
| 更新者／更新日 | modernizeX／2026-08-30 |

承認枠: お客様｜作成者｜承認者｜責任者

本書は SAKURA 販売管理システムのオンライン画面アプリ（ルート `MENU00`、66 メンバー）を 1 冊で
被覆する。メインメニュー（`MENU00`）から動的に呼び出される 36 の画面プログラムを §2〜§4 に
画面 ID ごとのブロックで、メニューから起動されるバッチ型メンバー（帳票・締め）と共通サブルーチンを
§1.2 のメンバー一覧で記述する。
...
> システム構成: メインメニューがオペレーターを認証し、選択された業務画面を動的に呼び出す。
> 各業務画面は共通サブルーチンを利用し、索引編成ファイル群を読み書きする。
...
---
flowchart LR
    OP["オペレーター"]:::actor --> MENU["MENU00<br>サインオン・メインメニュー"]:::screen
    MENU -- "ログイン検証" --> CHK["CHKLOG<br>ログイン／権限検証"]:::utility
    MENU -- "動的呼出し（メニュー選択）" --> BIZ["業務画面 36 本<br>マスタ保守／受注・売上／購買／在庫／債権債務"]:::screen
    MENU -- "動的呼出し" --> BAT["帳票・締めバッチ<br>RP0010–0140／BT0010–0090／OE0050"]:::batch
    BIZ -- "共通処理" --> UTIL["DATEUT／NUMGEN／TAXCAL／CREDIT／GETMSG／ABORTX"]:::utility
    BIZ --> MST[("マスタ群<br>得意先・仕入先・商品・在庫 ほか")]:::app
    BIZ --> TRN[("取引・元帳群<br>受注・売上・発注・仕入・出荷・入荷・売掛・買掛")]:::app
    BAT --> REP[("帳票ファイル REPF／ログ LOGF")]:::app
    classDef online fill:#cfe8ff,stroke:#1976d2,color:#000
    classDef batch fill:#ffe0b2,stroke:#e65100,color:#000
    classDef screen fill:#d4edda,stroke:#2e7d32,color:#000
    classDef utility fill:#ede7f6,stroke:#5e35b1,color:#000
    classDef actor fill:#fff,stroke:#555,stroke-dasharray: 3 3,color:#000
    classDef app fill:#d4edda,stroke:#2e7d32
```

### 1.2 機能概要

**ロール**: 全オペレーターはメインメニューでログイン（ログイン ID＋パスワード）し、ユーザーマスタの
5 種の権限フラグ（マスタ／受注／売上／購買／締め）で各業務の操作可否が判定される。
...
6. 帳票 — 各種一覧・集計・元帳・レポートの印刷（帳票ファイルへ出力）。
7. バッチ・締め — 日次／月次／年次の締め、残高再計算、パージ、再編成、整合性チェック。

**ファンクションキー**（本システム共通の操作規約。各画面の下部にキー凡例を表示）:

| キー | 表示ラベル（逐語, 代表例） | 動作（画面により表記が異なる。詳細は §4） |
|---|---|---|
| `ENTER` | `ENTER=Read` | 読込・次明細・検索・確定など画面ごとの主操作（ENTER=Next／ENTER=Search／ENTER=Confirm 等） |
| `PF3` | `PF3=End` | 業務を終了しメニューへ戻る（PF3=End/Finish／PF3=Done 等） |
| `PF9` | `PF9=Delete` | マスタ保守画面で表示中レコードを削除対象にする |
| `PF4` | `PF4=Clear line` | 明細クリア・スキップ・新規など補助操作（PF4=Clear／PF4=Skip line／PF4=New 等） |
| `PF6` | `PF6=NextPage` | 一覧・照会画面のページ送り |
| `PF12` | `PF12=PrevPage` | 一覧・照会画面のページ戻し |
| `PF5/6` | `PF5/6=Prev/Next` | 照会画面で前後レコードへ移動する |

各キーの画面別の具体挙動は §4 イベント仕様（画面 ID ごと）を参照。

**メンバー一覧**（アプリ全体 66 メンバー。画面プログラムは §2〜§4 で詳述、バッチ型・サブは下表で記述）:

| 種別 | プログラムID | 業務上の役割 | 主な I/O | 画面 |
|---|---|---|---|---|
| ルート | MENU00 | サインオン・メインメニュー（動的呼出し） | USERF（ログイン照合） | 有（§2.1） |
| 画面 | MS0010〜MS0120 | 各種マスタ保守（12 本） | 各マスタ C/R/U/D | 有（§2.2〜2.13） |
| 画面 | OE0010〜OE0040 | 受注 入力／照会／引当／保守 | 受注・在庫 ほか | 有（§2.14〜2.17） |
| 画面 | SL0010〜SL0040 | 売上 入力／照会／返品／クレジットノート | 売上・売掛・在庫 | 有（§2.18〜2.21） |
| 画面 | SH0010 | 出荷入力（ピッキング／出庫） | 出荷・在庫・受注 | 有（§2.22） |
| 画面 | PU0010〜PU0040 | 発注入力／照会、仕入入力／返品 | 発注・仕入・買掛・在庫 | 有（§2.23〜2.26） |
| 画面 | RC0010 | 入荷入力 | 入荷・発注・在庫 | 有（§2.27） |
| 画面 | IV0010〜IV0050 | 在庫 照会／調整／棚卸／移動履歴／倉庫間移動 | 在庫・移動履歴 | 有（§2.28〜2.32） |
| 画面 | AR0010/AR0020 | 入金入力、売掛照会・年齢表 | 入金・売掛元帳 | 有（§2.33〜2.34） |
| 画面 | AP0010/AP0020 | 支払入力、買掛照会 | 支払・買掛元帳 | 有（§2.35〜2.36） |
| バッチ | OE0050 | 受注確認書・ピッキングリスト印刷 | 受注 → 帳票 | 無 |
| バッチ | RP0010〜RP0140 | 各種帳票（一覧・集計・元帳・レポート 14 本） | 各ファイル → 帳票 REPF | 無 |
| バッチ | BT0010〜BT0090 | 締め・残高再計算・パージ・再編成・整合性チェック（9 本） | 各取引・在庫ファイル | 無 |
| サブ | CHKLOG | ログイン／権限検証 | USERF | 無 |
| サブ | DATEUT | 日付ユーティリティ（TODY/VALD/ADDD/DIFF/EOM/YOBI） | - | 無 |
| サブ | NUMGEN | 採番（次番号払出し） | NUMCF | 無 |
| サブ | TAXCAL | 消費税計算 | TAXF | 無 |
| サブ | CREDIT | 与信限度チェック | CUSTF | 無 |
| サブ | GETMSG | メッセージ文言取得（`Batch/GETMSG_…` に詳細） | MSGF | 無 |
| サブ | ABORTX | 異常終了ロガー | LOGF（ABEND.LOG） | 無 |

**帳票バッチ（メニュー「6. Reports」から起動、画面なし）の内訳**: OE0050（受注確認書・ピッキングリスト印刷）、
RP0010（得意先マスタ一覧）、RP0020（商品マスタ一覧）、RP0030（売上日記帳）、RP0040（得意先別売上集計）、
RP0050（商品別売上集計）、RP0060（在庫評価一覧）、RP0070（売掛年齢表）、RP0080（買掛残高一覧）、
RP0090（受注残一覧）、RP0100（仕入日記帳）、RP0110（発注推奨レポート）、RP0120（営業担当実績レポート）、
RP0130（得意先元帳）、RP0140（デッド／滞留在庫レポート）。

**締め・保守バッチ（メニュー「7. Batch / Closing」から起動、画面なし）の内訳**: BT0010（日次締め・売上計上）、
BT0020（月次締め）、BT0030（在庫月次更新・評価）、BT0040（売掛残高再計算）、BT0050（買掛残高再計算）、
BT0060（パージ／アーカイブ）、BT0070（会計年度更新）、BT0080（再編成／リオーグ）、BT0090（データ整合性チェック）。

### 1.3 使用データベース

> 本アプリの全画面が使用するテーブルと、アプリ全体での CRUD（画面プログラム群の実動詞の集約）。

| № | 論理テーブル名 | 物理テーブル名 | 登録(C) | 参照(R) | 更新(U) | 削除(D) | 備考 |
|---|---|---|---|---|---|---|---|
| 1 | 得意先マスタ | CUSTF | 〇 | 〇 | 〇 | - | 取引計上時に残高を更新（U） |
| 2 | 仕入先マスタ | SUPPF | 〇 | 〇 | 〇 | - | 取引計上時に残高を更新（U） |
| 3 | 商品マスタ | PRODF | 〇 | 〇 | 〇 | - | |
| 4 | 倉庫マスタ | WHSEF | 〇 | 〇 | 〇 | - | |
| 5 | 社員マスタ | STAFF | 〇 | 〇 | 〇 | - | |
| 6 | 商品分類マスタ | CATGF | 〇 | 〇 | 〇 | - | |
| 7 | 得意先別単価マスタ | CPRCF | 〇 | 〇 | 〇 | - | |
| 8 | 部門マスタ | DEPTF | 〇 | 〇 | 〇 | - | |
| 9 | 消費税率マスタ | TAXF | 〇 | 〇 | 〇 | 〇 | 保守で物理削除あり |
| 10 | 銀行マスタ | BANKF | 〇 | 〇 | 〇 | - | |
| 11 | 地域マスタ | REGNF | 〇 | 〇 | 〇 | - | |
| 12 | ユーザーマスタ | USERF | 〇 | 〇 | 〇 | - | ログイン照合（R） |
| 13 | 受注ヘッダ | ORDHF | 〇 | 〇 | 〇 | - | |
| 14 | 受注明細 | ORDDF | 〇 | 〇 | 〇 | 〇 | 受注保守で明細削除 |
| 15 | 売上ヘッダ | INVHF | 〇 | 〇 | - | - | |
| 16 | 売上明細 | INVDF | 〇 | 〇 | - | - | |
| 17 | 発注ヘッダ | POHF | 〇 | 〇 | 〇 | - | |
| 18 | 発注明細 | PODF | 〇 | 〇 | 〇 | - | |
| 19 | 仕入ヘッダ | PURHF | 〇 | - | - | - | 追記のみ |
| 20 | 仕入明細 | PURDF | 〇 | - | - | - | 追記のみ |
| 21 | 入荷ヘッダ | RCVHF | 〇 | 〇 | 〇 | - | |
| 22 | 入荷明細 | RCVDF | 〇 | 〇 | - | - | |
| 23 | 出荷ヘッダ | SHPHF | 〇 | 〇 | 〇 | - | |
| 24 | 出荷明細 | SHPDF | 〇 | 〇 | - | - | |
| 25 | 在庫残高 | STOKF | 〇 | 〇 | 〇 | - | 引当・調整・棚卸・移動で更新 |
| 26 | 在庫移動履歴 | SMOVF | 〇 | 〇 | - | - | 追記のみ |
| 27 | 売掛元帳 | ARLF | 〇 | 〇 | - | - | 追記のみ |
| 28 | 買掛元帳 | APLF | 〇 | 〇 | - | - | 追記のみ |
| 29 | 入金 | RCPTF | 〇 | - | - | - | 追記のみ |
| 30 | 支払 | PAYF | 〇 | - | - | - | 追記のみ |
...
> 本節はアプリ全体の画面遷移図で始まり、続いて**画面 ID ごとに 1 ブロック**（2.1〜2.36）で
> レイアウト（AS-IS モックアップ）・項目詳細・項目状態を記述する。各ブロックの番号は
> §3 チェック仕様・§4 イベント仕様と一致する（2.n・3.n・4.n は同じ画面）。
...
---
flowchart TB
    LOGIN["MENU00<br>ログイン画面"]:::screen -- "ログイン成功" --> MAIN["MENU00<br>メインメニュー"]:::screen
    LOGIN -- "ログイン失敗（3 回）／PF3" --> END["終了（サインオフ）"]:::screen
    MAIN -- "1 選択" --> SMST["サブメニュー<br>マスタ保守"]:::screen
    MAIN -- "2 選択" --> SORD["サブメニュー<br>受注／売上"]:::screen
    MAIN -- "3 選択" --> SPUR["サブメニュー<br>購買"]:::screen
    MAIN -- "4 選択" --> SINV["サブメニュー<br>在庫"]:::screen
    MAIN -- "6 選択" --> SRPT["サブメニュー<br>帳票"]:::batch
    MAIN -- "7 選択" --> SBAT["サブメニュー<br>バッチ／締め"]:::batch
    MAIN -- "0 選択" --> END
    SMST -- "番号選択→動的呼出し" --> MSSCR["MS0010〜MS0120<br>各マスタ保守"]:::screen
    SORD -- "番号選択" --> OESCR["OE0010〜OE0040／SL0010〜SL0040／SH0010"]:::screen
    SPUR -- "番号選択" --> PUSCR["PU0010〜PU0040／RC0010"]:::screen
    SINV -- "番号選択" --> IVSCR["IV0010〜IV0050"]:::screen
    SORD -- "債権債務" --> ARSCR["AR0010／AR0020／AP0010／AP0020"]:::screen
    SRPT -- "番号選択" --> RPBAT["RP0010〜RP0140<br>帳票印刷"]:::batch
    SBAT -- "番号選択" --> BTBAT["BT0010〜BT0090<br>締め・保守"]:::batch
    MSSCR -- "PF3=End" --> MAIN
    OESCR -- "PF3=End" --> MAIN
    PUSCR -- "PF3=End" --> MAIN
    IVSCR -- "PF3=End" --> MAIN
...
- **MENU00 — ログイン画面**（レコード DS-LOGIN） — 起動直後に表示され、ログイン ID とパスワードを入力する。

![AS-IS MENU00 login 画面](../Image/MENU00_login.png)

- **MENU00 — メインメニュー**（レコード DS-MAIN） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MENU00 main 画面](../Image/MENU00_main.png)

- **MENU00 — サブメニュー（マスタ保守）**（レコード DS-MENU-MST） — メインメニューで「1. Master Maintenance」を選ぶと表示される。

![AS-IS MENU00 menu_mst 画面](../Image/MENU00_menu_mst.png)

- **MENU00 — サブメニュー（受注／売上）**（レコード DS-MENU-ORD） — メインメニューで「2. Order / Sales」を選ぶと表示される。

![AS-IS MENU00 menu_ord 画面](../Image/MENU00_menu_ord.png)

- **MENU00 — サブメニュー（購買）**（レコード DS-MENU-PUR） — メインメニューで「3. Purchasing」を選ぶと表示される。

![AS-IS MENU00 menu_pur 画面](../Image/MENU00_menu_pur.png)

- **MENU00 — サブメニュー（在庫）**（レコード DS-MENU-INV） — メインメニューで「4. Inventory」を選ぶと表示される。

![AS-IS MENU00 menu_inv 画面](../Image/MENU00_menu_inv.png)

- **MENU00 — サブメニュー（帳票）**（レコード DS-MENU-RPT） — メインメニューで「6. Reports」を選ぶと表示される。

![AS-IS MENU00 menu_rpt 画面](../Image/MENU00_menu_rpt.png)

- **MENU00 — サブメニュー（バッチ／締め）**（レコード DS-MENU-BAT） — メインメニューで「7. Batch / Closing」を選ぶと表示される。

![AS-IS MENU00 menu_bat 画面](../Image/MENU00_menu_bat.png)
...
- **MS0010 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0010 main 画面](../Image/MS0010_main.png)
...
- **MS0020 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0020 main 画面](../Image/MS0020_main.png)
...
- **MS0030 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0030 main 画面](../Image/MS0030_main.png)
...
- **MS0040 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0040 main 画面](../Image/MS0040_main.png)
...
- **MS0050 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0050 main 画面](../Image/MS0050_main.png)
...
- **MS0060 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0060 main 画面](../Image/MS0060_main.png)
...
- **MS0070 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0070 main 画面](../Image/MS0070_main.png)
...
- **MS0080 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0080 main 画面](../Image/MS0080_main.png)
...
- **MS0090 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0090 main 画面](../Image/MS0090_main.png)
...
- **MS0100 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0100 main 画面](../Image/MS0100_main.png)
...
- **MS0110 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0110 main 画面](../Image/MS0110_main.png)
...
- **MS0120 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS MS0120 main 画面](../Image/MS0120_main.png)
...
- **OE0010 — 入力画面**（レコード DS-HEAD） — 業務開始時に表示され、ヘッダと明細を続けて入力する。

![AS-IS OE0010 entry 画面](../Image/OE0010_entry.png)
...
- **OE0020 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS OE0020 key 画面](../Image/OE0020_key.png)

- **OE0020 — 照会画面**（レコード DS-ORDER） — キー入力後、該当データが見つかると表示される。

![AS-IS OE0020 view 画面](../Image/OE0020_view.png)
...
- **OE0030 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS OE0030 key 画面](../Image/OE0030_key.png)

- **OE0030 — 照会画面**（レコード DS-ORDER） — キー入力後、該当データが見つかると表示される。

![AS-IS OE0030 view 画面](../Image/OE0030_view.png)
...
- **OE0040 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS OE0040 key 画面](../Image/OE0040_key.png)

- **OE0040 — 照会画面**（レコード DS-INFO） — キー入力後、該当データが見つかると表示される。

![AS-IS OE0040 view 画面](../Image/OE0040_view.png)

- **OE0040 — 受注編集画面**（レコード DS-HDR） — 受注番号入力後、ヘッダ・明細の編集画面が表示される。

![AS-IS OE0040 edit 画面](../Image/OE0040_edit.png)

- **OE0040 — 明細変更画面**（レコード DS-CHGLINE） — 一覧で明細を選択し、変更操作を行うと表示される。

![AS-IS OE0040 chgline 画面](../Image/OE0040_chgline.png)
...
- **SL0010 — 入力画面**（レコード DS-KEY） — 業務開始時に表示され、ヘッダと明細を続けて入力する。

![AS-IS SL0010 entry 画面](../Image/SL0010_entry.png)

- **SL0010 — 照会画面**（レコード DS-INV） — キー入力後、該当データが見つかると表示される。

![AS-IS SL0010 view 画面](../Image/SL0010_view.png)
...
- **SL0020 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS SL0020 key 画面](../Image/SL0020_key.png)

- **SL0020 — 照会画面**（レコード DS-INV） — キー入力後、該当データが見つかると表示される。

![AS-IS SL0020 view 画面](../Image/SL0020_view.png)
...
- **SL0030 — 入力画面**（レコード DS-KEY） — 業務開始時に表示され、ヘッダと明細を続けて入力する。

![AS-IS SL0030 entry 画面](../Image/SL0030_entry.png)

- **SL0030 — 照会画面**（レコード DS-INV） — キー入力後、該当データが見つかると表示される。

![AS-IS SL0030 view 画面](../Image/SL0030_view.png)
...
- **SL0040 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS SL0040 key 画面](../Image/SL0040_key.png)

- **SL0040 — ピッキング画面**（レコード DS-INFO） — 番号入力後、値引／ピッキング対象の明細が一覧表示される。

![AS-IS SL0040 pick 画面](../Image/SL0040_pick.png)
...
- **SH0010 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS SH0010 key 画面](../Image/SH0010_key.png)

- **SH0010 — 出荷入力画面**（レコード DS-ORDER） — 受注番号入力後、出荷対象の明細が表示される。

![AS-IS SH0010 ship 画面](../Image/SH0010_ship.png)
...
- **PU0010 — 入力画面**（レコード DS-HEAD） — 業務開始時に表示され、ヘッダと明細を続けて入力する。

![AS-IS PU0010 entry 画面](../Image/PU0010_entry.png)
...
- **PU0020 — 検索画面**（レコード DS-SEARCH） — 業務開始時に表示され、検索条件を入力する。

![AS-IS PU0020 search 画面](../Image/PU0020_search.png)

- **PU0020 — 照会画面**（レコード DS-HDRVIEW） — キー入力後、該当データが見つかると表示される。

![AS-IS PU0020 view 画面](../Image/PU0020_view.png)
...
- **PU0030 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS PU0030 key 画面](../Image/PU0030_key.png)

- **PU0030 — 仕入計上画面**（レコード DS-RINFO） — 入荷番号入力後、仕入計上対象の明細が表示される。

![AS-IS PU0030 booking 画面](../Image/PU0030_booking.png)
...
- **PU0040 — 入力画面**（レコード DS-HEAD） — 業務開始時に表示され、ヘッダと明細を続けて入力する。

![AS-IS PU0040 entry 画面](../Image/PU0040_entry.png)
...
- **RC0010 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS RC0010 key 画面](../Image/RC0010_key.png)

- **RC0010 — 入荷入力画面**（レコード DS-PINFO） — 発注番号入力後、未入荷明細が表示される。

![AS-IS RC0010 recv 画面](../Image/RC0010_recv.png)
...
- **IV0010 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS IV0010 key 画面](../Image/IV0010_key.png)

- **IV0010 — 明細照会画面**（レコード DS-DETAIL） — 検索実行後、該当在庫が見つかると 1 件ずつ表示される。

![AS-IS IV0010 detail 画面](../Image/IV0010_detail.png)
...
- **IV0020 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS IV0020 main 画面](../Image/IV0020_main.png)
...
- **IV0030 — 倉庫選択画面**（レコード DS-WH） — 業務開始時に対象倉庫を選択する。

![AS-IS IV0030 wh 画面](../Image/IV0030_wh.png)

- **IV0030 — 棚卸入力画面**（レコード DS-COUNT） — 倉庫選択後、商品ごとの棚卸数を入力する。

![AS-IS IV0030 count 画面](../Image/IV0030_count.png)

- **IV0030 — 棚卸確認画面**（レコード DS-REVIEW） — 棚卸入力後、確定前に内容を確認表示する。

![AS-IS IV0030 review 画面](../Image/IV0030_review.png)
...
- **IV0040 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS IV0040 key 画面](../Image/IV0040_key.png)

- **IV0040 — 一覧画面**（レコード DS-LIST） — キー入力後、該当明細が見つかると一覧表示される。

![AS-IS IV0040 list 画面](../Image/IV0040_list.png)
...
- **IV0050 — 保守画面**（レコード DS-KEY） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS IV0050 main 画面](../Image/IV0050_main.png)
...
- **AR0010 — 保守画面**（レコード DS-HEAD） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS AR0010 main 画面](../Image/AR0010_main.png)
...
- **AR0020 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS AR0020 key 画面](../Image/AR0020_key.png)

- **AR0020 — 年齢表画面**（レコード DS-AGE） — 得意先コード入力後、売掛残高と年齢表が表示される。

![AS-IS AR0020 aging 画面](../Image/AR0020_aging.png)
...
- **AP0010 — 保守画面**（レコード DS-HEAD） — 業務開始時にキーを入力し、対象レコードを読み込むと保守項目が表示される。

![AS-IS AP0010 main 画面](../Image/AP0010_main.png)
...
- **AP0020 — キー入力画面**（レコード DS-KEY） — 業務開始時に表示され、検索キーを入力する。

![AS-IS AP0020 key 画面](../Image/AP0020_key.png)

- **AP0020 — 照会画面**（レコード DS-VIEW） — キー入力後、該当データが見つかると表示される。

![AS-IS AP0020 view 画面](../Image/AP0020_view.png)
...
> **画面 ID ごとに 1 ブロック（4.1〜4.36、番号は §2・§3 と一致）。** キー／項目ごとに、契機・
> 動作・遷移先を業務のことばで記す。ファンクションキーは画面下部に表示される凡例（逐語）を
> 分解して 1 キー 1 行で示す。


### 4.1 MENU00 — サインオン・メインメニュー

| No. | 項目 | タイミング／条件 | 動作 | 備考 |
|---|---|---|---|---|
| **ログイン部** |||||
| 1 | ログイン ID／パスワード | 入力時 | 入力されたログイン ID とパスワードを照合し、正しければメインメニューへ進む。誤っていれば再入力を促し、上限回数を超えると終了する | メッセージ L172／L188／L191 |
| 2 | `PF3` | ログイン画面での押下時 | ログインを中止してシステムを終了する | fkey凡例なし。ログインメッセージ内 `PF3 to quit`（L172）に準拠 |
| **メインメニュー部** |||||
| 3 | 業務グループ番号 | 入力時 | 入力された業務グループ番号に対応するサブメニュー（マスタ保守／受注・売上／購買／在庫／帳票／バッチ・締め）へ進む。範囲外なら再入力を促す | メッセージ L222。「7. Batch / Closing」選択時は権限を判定（L366） |
| 4 | `0`（サインオフ） | メインメニューでの入力時 | 業務を終了してサインオフする | メッセージ L471 |
| **サブメニュー部** |||||
| 5 | サブメニュー番号 | 入力時 | 入力された番号に対応する業務プログラムを起動する。範囲外なら再入力を促し、未提供なら起動しない | メッセージ L397／L462 |
| 6 | 業務プログラム起動 | サブメニュー番号確定時 | 選択された業務プログラムへ制御を渡し、終了後はサブメニューへ戻る | 呼出先は得意先マスタ保守～各バッチ処理 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した得意先コードで得意先マスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の得意先を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **キー入力部** |||||
| 4 | 得意先コード | 入力時（ENTER） | 得意先マスタを照合し、既存なら明細を表示、未登録／論理削除済みなら新規登録へ切り替える。コードが 0 の場合は受け付けない | メッセージ L169／L189／L197／L200 |
| **明細入力部** |||||
| 5 | 得意先明細（氏名・地域・担当社員・銀行・与信限度額 ほか） | 入力時（ENTER） | 得意先名の必須、地域・担当社員・銀行の各コードのマスタ存在、与信限度額の非負を検証し、不備があれば当該項目を促す | 地域マスタ・社員マスタ・銀行マスタで検証 |
| **確認部** |||||
| 6 | `Confirm (Y/N)` | 登録／変更の入力時 | `Y` で登録／更新を確定し、得意先マスタへ記帳する | 新規は登録、既存は更新 |
| 7 | `Confirm (Y/N)` | 削除操作（PF9）後の入力時 | `Y` で論理削除を確定し得意先マスタへ反映する。`Y` 以外なら削除を取りやめる | メッセージ L317／L332 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した仕入先コードで仕入先マスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の仕入先を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **キー入力部** |||||
| 4 | 仕入先コード | 入力時（ENTER） | 仕入先マスタを照合し、既存なら明細を表示、未登録／論理削除済みなら新規登録へ切り替える。コードが 0 の場合は受け付けない | メッセージ L164／L184／L192／L195 |
| **明細入力部** |||||
| 5 | 仕入先明細（氏名・締日・支払日・支払方法・税区分・銀行 ほか） | 入力時（ENTER） | 仕入先名の必須、締日・支払日の 1〜31／99、支払方法・税区分の 1〜3、銀行コードの銀行マスタ存在を検証し、不備があれば当該項目を促す | 銀行マスタで検証 |
| **確認部** |||||
| 6 | `Confirm (Y/N)` | 登録／変更の入力時 | `Y` で登録／更新を確定し、仕入先マスタへ記帳する | 新規は登録、既存は更新 |
| 7 | `Confirm (Y/N)` | 削除操作（PF9）後の入力時 | `Y` で論理削除を確定し仕入先マスタへ反映する。`Y` 以外なら削除を取りやめる | メッセージ L313／L328 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した商品コードで商品マスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の商品を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **キー入力部** |||||
| 4 | 商品コード | 入力時（ENTER） | 商品マスタを照合し、既存なら明細を表示、未登録／論理削除済みなら新規登録へ切り替える。コードが 0 の場合は受け付けない | メッセージ L196／L218／L226／L229 |
| **明細入力部** |||||
| 5 | 商品明細（商品名・分類・税区分・在庫管理区分・原価・定価・ランク単価・在庫水準・発注点・既定仕入先・既定倉庫 ほか） | 入力時（ENTER） | 商品名・分類の必須、分類コードの分類マスタ存在、税区分の 1〜3、在庫管理区分の 0／1、各金額・数量の非負を検証し、既定仕入先・既定倉庫の各マスタ存在を確認する。不備があれば当該項目を促す | 商品分類マスタ・仕入先マスタ・倉庫マスタで検証 |
| **確認部** |||||
| 6 | `Confirm (Y/N)` | 登録／変更の入力時 | `Y` で登録／更新を確定し、商品マスタへ記帳する | 新規は登録、既存は更新 |
| 7 | `Confirm (Y/N)` | 削除操作（PF9）後の入力時 | `Y` で論理削除を確定し商品マスタへ反映する。`Y` 以外なら削除を取りやめる | メッセージ L402／L417 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した倉庫コードで倉庫マスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の倉庫を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **キー入力部** |||||
| 4 | 倉庫コード | 入力時（ENTER） | 倉庫マスタを照合し、既存なら明細を表示、未登録／論理削除済みなら新規登録へ切り替える。コードが 0 の場合は受け付けない | メッセージ L135／L156／L164／L167 |
| **明細入力部** |||||
| 5 | 倉庫明細（倉庫名・倉庫区分・管理者コード ほか） | 入力時（ENTER） | 倉庫名の必須、倉庫区分の 1〜3、管理者（社員）コードの社員マスタ存在を検証し、不備があれば当該項目を促す | 社員マスタで検証 |
| **確認部** |||||
| 6 | `Confirm (Y/N)` | 登録／変更の入力時 | `Y` で登録／更新を確定し、倉庫マスタへ記帳する | 新規は登録、既存は更新 |
| 7 | `Confirm (Y/N)` | 削除操作（PF9）後の入力時 | `Y` で論理削除を確定し倉庫マスタへ反映する。`Y` 以外なら削除を取りやめる | メッセージ L262／L275 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した社員コードで社員マスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の社員を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **キー入力部** |||||
| 4 | 社員コード | 入力時（ENTER） | 社員マスタを照合し、既存なら明細を表示、未登録／論理削除済みなら新規登録へ切り替える。コードが 0 の場合は受け付けない | メッセージ L133／L153／L161／L164 |
| **明細入力部** |||||
| 5 | 社員明細（社員名・部門 ほか） | 入力時（ENTER） | 社員名・部門の必須、部門コードの部門マスタ存在と当該部門が論理削除済みでないことを検証し、不備があれば当該項目を促す | 部門マスタで検証 |
| **確認部** |||||
| 6 | `Confirm (Y/N)` | 登録／変更の入力時 | `Y` で登録／更新を確定し、社員マスタへ記帳する | 新規は登録、既存は更新 |
| 7 | `Confirm (Y/N)` | 削除操作（PF9）後の入力時 | `Y` で論理削除を確定し社員マスタへ反映する。`Y` 以外なら削除を取りやめる | メッセージ L262／L275 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した分類コードで商品分類マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の商品分類を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 分類コード | 入力時（ENTER） | 入力コードで商品分類マスタを照合し、該当区分（新規／再登録／既存）を判定して明細を表示する | 0 のとき `Category code must not be zero` |
| **明細項目部** |||||
| 5 | 分類名 | 明細入力時 | 未入力を許さず必須チェックする | `Name is required` |
| 6 | 親分類コード | 明細入力時 | 自己参照禁止・親の存在・親の削除状態を検証する（親分類は同一マスタへの自己参照） | `Parent cannot be itself` / `Parent category not found` / `Parent category is deleted` |
| **確定部** |||||
| 7 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し商品分類マスタへ記帳、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 8 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Category added` / `Category updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した得意先コード＋商品コードの複合キーで得意先別単価マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の得意先別単価を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 得意先コード | 入力時（ENTER） | 得意先マスタを照合し、存在・削除状態を確認する（複合キーの第1要素） | 0 のとき `Customer code must not be zero`／`Customer code not found`／`Customer is deleted` |
| 5 | 商品コード | 入力時（ENTER） | 商品マスタを照合し、存在・削除状態を確認する（複合キーの第2要素） | 0 のとき `Product code must not be zero`／`Product code not found`／`Product is deleted` |
| **明細項目部** |||||
| 6 | 単価 | 明細入力時 | 負数を許さず検証する | `Price cannot be negative` |
| 7 | 適用開始日／適用終了日 | 明細入力時 | 開始日の必須・暦日妥当性、終了日の暦日妥当性、および終了日が開始日以降であることを検証する | `Start date is required`／`Start date is invalid`／`End date is invalid`／`End date is before start date` |
| **確定部** |||||
| 8 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し得意先別単価マスタへ記帳、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 9 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Contract price added` / `Contract price updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した部門コードで部門マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の部門を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 部門コード | 入力時（ENTER） | 入力コードで部門マスタを照合し、該当区分（新規／再登録／既存）を判定して明細を表示する | 0 のとき `Department code must not be zero` |
| **明細項目部** |||||
| 5 | 部門名 | 明細入力時 | 未入力を許さず必須チェックする | `Name is required` |
| 6 | 親部門コード | 明細入力時 | 自己参照禁止・親の存在を検証する（親部門は同一マスタへの自己参照） | `Parent cannot be the department itself` / `Parent department not found` |
| **確定部** |||||
| 7 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し部門マスタへ記帳、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 8 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Department added` / `Department updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した税区分コード＋適用開始日の複合キーで消費税率マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の税率を削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 税区分コード | 入力時（ENTER） | 1〜9 の範囲を検証する（複合キーの第1要素） | 範囲外は `Tax category must be 1 to 9` |
| 5 | 適用開始日 | 入力時（ENTER） | 必須および暦日妥当性を検証する（複合キーの第2要素、日付検証は DATEUT で実施） | `Start date is required` / `Start date is invalid` |
| **明細項目部** |||||
| 6 | 税率 | 明細入力時 | 負数不可かつ 1.000 未満の小数であることを検証する | `Rate cannot be negative` / `Rate must be a fraction below 1.000` |
| 7 | 税率名 | 明細入力時 | 未入力を許さず必須チェックする | `Rate name is required` |
| **確定部** |||||
| 8 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し消費税率マスタから該当を削除、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 9 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Tax rate added` / `Tax rate updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した銀行コードで銀行マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の銀行を論理削除の対象にする（削除フラグによる論理削除） | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 銀行コード | 入力時（ENTER） | 入力コードで銀行マスタを照合し、該当区分（新規／再登録／既存）を判定して明細を表示する | 0 のとき `Bank code must not be zero` |
| **明細項目部** |||||
| 5 | 銀行名 | 明細入力時 | 未入力を許さず必須チェックする | `Bank name is required` |
| **確定部** |||||
| 6 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し銀行マスタへ記帳、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 7 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Bank added` / `Bank updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した地域コードで地域（営業テリトリ）マスタを読み、登録済みなら変更／削除、未登録なら新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中の地域を論理削除の対象にする | PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 4 | 地域コード | 入力時（ENTER） | 入力コードで地域マスタを照合し、該当区分（新規／再登録／既存）を判定して明細を表示する | 0 のとき `Region code must not be zero` |
| **明細項目部** |||||
| 5 | 地域名 | 明細入力時 | 未入力を許さず必須チェックする | `Region name is required` |
| **確定部** |||||
| 6 | `Confirm (Y/N)` | 削除確認入力時 | `Y` で削除を確定し地域マスタへ記帳、`Y` 以外なら削除を取りやめる | `Press Y then ENTER to delete` |
| 7 | 登録／更新確定 | 明細確定時 | 新規は追加記帳、既存は変更内容で更新記帳する | `Region added` / `Region updated` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力したユーザーコードでユーザーマスタを読み、存在すれば変更／削除、無ければ新規登録に切り替える | fkey凡例: ENTER=Read |
| 2 | `PF9` | 押下時 | 表示中のユーザーを論理削除の対象にする | fkey凡例: PF9=Delete |
| 3 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | fkey凡例: PF3=End |
| **ヘッダ項目部** |||||
| 4 | ユーザーコード | 入力し ENTER 押下時 | 入力コードでユーザーマスタを照合し、区分に応じて新規／変更／削除画面へ進む | 該当時 `Existing user - change or PF9 delete`／未登録時 `New user - enter details` |
| 5 | ログイン ID・氏名・ロール・権限フラグ | 入力時 | ログイン ID・氏名・ロール（1/2/3）・5 種の権限フラグ（0/1）を保守項目として受け付ける | 管理者以外は社員マスタと突合 |
| 6 | 登録／更新確定 | 入力完了時 | 全項目の妥当性を判定のうえユーザーマスタへ記帳する | 成功時 `User added`／`User updated` |
| 7 | `Press Y then ENTER to delete` | 削除確認入力時 | `Y` を入力し ENTER で削除を確定し、ユーザーマスタを論理削除する | 成功時 `User deleted` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力中の行を確定し、次の明細行の入力へ進む | fkey凡例: ENTER=Next |
| 2 | `PF3` | 押下時 | 明細入力を締めて合計確認・確定へ進む、またはヘッダ段階では業務を終了する | fkey凡例: PF3=End/Finish |
| 3 | `PF4` | 押下時 | 入力中の明細行の入力内容をクリアする | fkey凡例: PF4=Clear line（`Line cleared`） |
| **ヘッダ項目部** |||||
| 4 | 得意先コード | 入力し ENTER 押下時 | 得意先マスタを照合し、有効な得意先なら明細入力へ進む（`Header OK - enter detail lines`） | 未登録・削除は再入力 |
| **明細項目部** |||||
| 5 | 商品コード・数量 | 明細行入力し ENTER 押下時 | 商品マスタを照合し、単価を得意先別単価または商品ランク単価から解決、在庫を引当し明細を 1 行追加する（`Line added`） | 数量が在庫超過時は警告表示 |
| **確定部** |||||
| 6 | 受注確定 | PF3 で明細を締めたとき | 合計・税を計算し確認のうえ、受注番号を採番して受注ヘッダ・明細を記帳する | 税計算・採番の共通処理を利用／成功時 `Order saved` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 表示中の受注明細の次ページを表示する | fkey凡例: ENTER=NextPage |
| 2 | `PF12` | 押下時 | 表示中の受注明細の前ページを表示する | fkey凡例: PF12=PrevPage |
| 3 | `PF5` | 押下時 | 得意先照会で 1 つ前の受注へ移動する | fkey凡例: PF5/6=Prev/Next（前受注） |
| 4 | `PF6` | 押下時 | 得意先照会で次の受注へ移動する | fkey凡例: PF5/6=Prev/Next（次受注） |
| 5 | `PF3` | 押下時 | 照会を終了しメニューへ戻る | fkey凡例: PF3=End |
| **検索条件部** |||||
| 6 | 受注番号 | 入力し ENTER 押下時 | 受注番号で受注ヘッダを直接読み、明細まで表示する | 未登録・取消／削除は再入力 |
| 7 | 得意先コード＋日付 | 入力し ENTER 押下時 | 得意先＋日付の代替キーで先頭受注を検索し、明細まで表示する | 該当無しは `No orders for this customer` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 受注番号入力画面で押下時 | 入力した受注番号で受注を読み込み、引当対象として表示する | fkey凡例（L258/L711）: ENTER=Read |
| 2 | `PF3` | 受注番号入力画面で押下時 | 引当業務を終了しメニューへ戻る | fkey凡例（L258/L711）: PF3=End |
| 3 | `PF6` | 明細一覧画面で押下時 | 明細の次ページを表示する | fkey凡例（L694）: PF6=NextPage |
| 4 | `PF12` | 明細一覧画面で押下時 | 明細の前ページを表示する | fkey凡例（L694）: PF12=PrevPage |
| 5 | `PF3` | 明細一覧画面で押下時 | 明細確認を終える | fkey凡例（L694）: PF3=Done |
| **入力・確定部** |||||
| 6 | 受注番号 | 入力し ENTER 押下時 | 受注ヘッダを読み、状態が引当可能で明細があることを確認して明細を表示する | 状態不可は `Order cannot be allocated in its status` |
| 7 | `Confirm allocation (Y) or PF3 to cancel` | 引当確認入力時 | `Y` で引当を確定し、明細ごとに在庫残高を読んで必要数と引当済を突き合わせ、引当数を確定して受注ヘッダ・明細・在庫残高を更新する | 成功時 `Allocation complete`／PF3 で `Allocation cancelled` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 受注番号入力画面で押下時 | 入力した受注番号で受注を読み込み保守画面へ進む | fkey凡例（L356/L610）: ENTER=Read |
| 2 | `PF3` | 受注番号入力画面で押下時 | 保守業務を終了しメニューへ戻る | fkey凡例（L356/L610）: PF3=End |
| 3 | `ENTER` | 保守画面で押下時 | 入力したコマンド（A/C/D/H/X/S）を実行する | fkey凡例（L586）: ENTER=Cmd |
| 4 | `PF6` | 保守画面で押下時 | 明細の次ページを表示する | fkey凡例（L586）: PF6/PF12=Page（`Last page`） |
| 5 | `PF12` | 保守画面で押下時 | 明細の前ページを表示する | fkey凡例（L586）: PF6/PF12=Page（`First page`） |
| 6 | `PF3` | 保守画面で押下時 | 保守を中断する（未保存があれば破棄確認） | fkey凡例（L586）: PF3=Quit（`Discard unsaved changes ? (Y/N)`） |
| **入力部（コマンド）** |||||
| 7 | 受注番号 | 入力し ENTER 押下時 | 受注ヘッダを読み、未出荷（状態 0 入力済／1 引当済）の間だけ保守可能とする | 読込後 `Order loaded - enter a command` |
| 8 | コマンド `A` | 明細追加コマンド実行時 | 商品・数量を検証して受注明細を 1 行追加する | 成功時 `Line added` |
| 9 | コマンド `C` | 明細変更コマンド実行時 | 対象行の数量・単価を検証して受注明細を変更する | 成功時 `Line changed` |
| 10 | コマンド `D` | 明細削除コマンド実行時 | 指定行番号の受注明細を削除する | 成功時 `Line deleted` |
| 11 | コマンド `H` | ヘッダ変更コマンド実行時 | 得意先・税区分などヘッダ項目を検証して変更する | 成功時 `Header changed` |
| **確定部** |||||
| 12 | コマンド `X` | 受注取消コマンド実行時 | `Cancel the WHOLE order ? (Y/N)` に `Y` で受注全体を取り消す | 成功時 `Order cancelled` |
| 13 | コマンド `S` | 保存コマンド実行時 | `Save the changes ? (Y/N)` に `Y` で確定し、税・与信を判定のうえ受注ヘッダ・明細・在庫残高へ記帳する | 与信超過時 `Warning: customer credit limit exceeded`／成功時 `Order saved` |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 検索・入力画面で押下時 | 入力した行を確定し、次の入力へ進む | fkey凡例（L321/L858）: ENTER=Next |
| 2 | `PF3` | 検索・入力画面で押下時 | 業務を終了する／入力を取り消す | fkey凡例（L321/L858）: PF3=End |
| 3 | `PF6` | 確認画面で押下時 | 明細の次ページを表示する | fkey凡例（L836）: PF6/PF12=page |
| 4 | `PF12` | 確認画面で押下時 | 明細の前ページを表示する | fkey凡例（L836）: PF6/PF12=page |
| 5 | `ENTER` | 確認画面で押下時 | 請求の計上を確定する | fkey凡例（L836）: ENTER=confirm（`Confirm to post the invoice`） |
| 6 | `PF3` | 確認画面で押下時 | 計上を取り消し請求を破棄する | fkey凡例（L836）: PF3=cancel（`Invoice cancelled`） |
| **検索条件部** |||||
| 7 | 出荷番号 | 入力し ENTER 押下時 | 既存出荷を読み込み、明細を複写して売上明細の初期値とする | 請求済・取消・削除は不可 |
| 8 | 得意先コード | 入力し ENTER 押下時 | 得意先マスタを照合し、直接キー入力で売上を作成する経路へ進む | 未登録・削除は再入力 |
| **ヘッダ・明細部** |||||
| 9 | 売上ヘッダ・税区分 | 入力し ENTER 押下時 | 税区分（1/2/3）などヘッダ項目を検証して受け付ける（`Enter sale header - PF3 to cancel`） | |
| 10 | 商品コード・数量 | 明細行入力し ENTER 押下時 | 商品マスタを照合し単価を解決して売上明細を 1 行追加する（`Line added`） | PF4 で明細行クリア（`Line cleared`） |
| **確定部** |||||
| 11 | 請求計上確定 | 確認画面で ENTER 押下時 | 税を計算し請求番号を採番して売上ヘッダ・明細を記帳し、売掛元帳（ARLF）へ計上して得意先残高を更新する | 税計算・採番の共通処理を利用 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 照会画面表示中の押下時 | 表示中の請求の明細を次ページへ送る | fkey凡例: ENTER=Page（L251）／メッセージ行 `ENTER/PF12 page`（L570） |
| 2 | `PF12` | 照会画面表示中の押下時 | 表示中の請求の明細を前ページへ戻す | fkey凡例: PF12=PrevPage（L251） |
| 3 | `PF5/6` | 照会画面表示中の押下時 | PF5 で前の請求、PF6 で次の請求へ参照対象を切り替える | fkey凡例: PF5/6=Prev/Next（L251）／`PF5/6 invoice`（L570） |
| 4 | `PF3` | 押下時 | 照会を終了しメニューへ戻る | fkey凡例: PF3=End（L251）／`PF3 back`（L570） |
| **キー入力部** |||||
| 5 | 請求番号 | 入力して `ENTER` 押下時 | 入力した請求番号で売上ヘッダを読み、該当があれば照会画面へ進む | 未存在時 `Invoice number not found`（L345） |
| 6 | 得意先コード | 請求番号を空にして入力し `ENTER` 押下時 | 得意先マスタを照合し、当該得意先の売上（請求）を代替キーで検索して照会画面へ進む | 未存在時 `Customer not found`（L365） |
...
|---|---|---|---|---|
| **ファンクション部（キー入力・ヘッダ）** |||||
| 1 | `ENTER` | キー入力／ヘッダ入力での押下時 | 入力内容を確定し次の入力段階へ進む | fkey凡例: ENTER=Next（L316, L748） |
| 2 | `PF3` | キー入力／ヘッダ入力での押下時 | 返品入力を中止しメニューへ戻る | fkey凡例: PF3=End（L316, L748）／`Return cancelled`（L734） |
| **ファンクション部（明細確定）** |||||
| 3 | `PF6` | 明細一覧表示中の押下時 | 明細一覧を次ページへ送る | fkey凡例: PF6/PF12=page（L726） |
| 4 | `PF12` | 明細一覧表示中の押下時 | 明細一覧を前ページへ戻す | fkey凡例: PF6/PF12=page（L726） |
| 5 | `ENTER` | 明細一覧表示中の押下時 | 返品計上の確認へ進む | fkey凡例: ENTER=confirm（L726）／`Confirm to post the return`（L756） |
| 6 | `PF3` | 明細一覧表示中の押下時 | 返品入力を取消す | fkey凡例: PF3=cancel（L726）／`Return discarded`（L763） |
| **キー・明細入力部** |||||
| 7 | 得意先コード | 入力して `ENTER` 押下時 | 得意先マスタを照合し、存在すれば返品ヘッダ入力へ進む | 未存在時 `Customer not found`（L492） |
| 8 | 商品コード・返品数量 | 明細行で入力して `ENTER` 押下時 | 商品マスタで商品を照合し、正の返品数量を明細に追加する（数量・金額は負値で保持） | 追加時 `Line added`（L683） |
| 9 | `Confirm to post the return` | 確認画面で `Y` 入力時 | 返品をマイナス請求として記帳し、返品品を在庫へ戻し、売掛元帳・得意先残高へ計上する | 返品番号は自動採番（NUMGEN） |
...
|---|---|---|---|---|
| **ファンクション部（キー入力）** |||||
| 1 | `ENTER` | キー入力画面での押下時 | 入力した元請求番号を読み込み、該当があれば明細ピック画面へ進む | fkey凡例: ENTER=Read（L275, L587） |
| 2 | `PF3` | キー入力画面での押下時 | クレジットノート入力を終了しメニューへ戻る | fkey凡例: PF3=End（L275, L587） |
| **ファンクション部（明細ピック）** |||||
| 3 | `ENTER` | 明細ピック画面での押下時 | 選択した元明細番号にクレジット数量を設定する | fkey凡例: ENTER=Set（L557）／`Credit quantity set`（L607） |
| 4 | `PF4` | 明細ピック画面での押下時 | 選択中の明細のクレジット数量を取消す | fkey凡例: PF4=ClrLine（L557）／`Credit quantity cleared`（L619） |
| 5 | `PF6` | 明細ピック画面での押下時 | 明細一覧を次ページへ送る | fkey凡例: PF6/PF12=Page（L557）／`Last page`（L982） |
| 6 | `PF12` | 明細ピック画面での押下時 | 明細一覧を前ページへ戻す | fkey凡例: PF6/PF12=Page（L557）／`First page`（L993） |
| 7 | `PF3` | 明細ピック画面での押下時 | 明細選択を終え、クレジット計上の確認へ進む | fkey凡例: PF3=Done（L557）／`Confirm to post the credit note`（L670） |
| **キー・明細入力部** |||||
| 8 | 元請求番号 | 入力して `ENTER` 押下時 | 売上ヘッダ・明細を読み込み、売上伝票であることを確認して明細を一覧表示する | 未存在時 `Original invoice not found`（L458） |
| 9 | 元明細番号・クレジット数量 | 明細ピック画面で入力して `ENTER` 押下時 | 請求済数量を上限にクレジット数量を設定する | 超過時 `Credit qty exceeds invoiced qty`（L603） |
| 10 | `Confirm to post the credit note` | 確認画面で `Y` 入力時 | クレジットノートを記帳し、対象数量を在庫へ戻し、売掛元帳・得意先残高へ計上する | クレジット番号は自動採番（NUMGEN） |
...
|---|---|---|---|---|
| **ファンクション部（キー入力）** |||||
| 1 | `ENTER` | キー入力画面での押下時 | 入力した受注番号を読み込み、引当済みなら出荷数入力画面へ進む | fkey凡例: ENTER=Read（L280, L610） |
| 2 | `PF3` | キー入力画面での押下時 | 出荷入力を終了しメニューへ戻る | fkey凡例: PF3=End（L280, L610） |
| **ファンクション部（出荷数入力）** |||||
| 3 | `ENTER` | 出荷数入力画面での押下時 | 指定した明細行の出荷数を設定する | fkey凡例: ENTER=set（L583）／`Line updated`（L674） |
| 4 | `PF4` | 出荷数入力画面での押下時 | 全明細の出荷数を 0 にする | fkey凡例: PF4=zero-all（L583） |
| 5 | `PF6` | 出荷数入力画面での押下時 | 明細一覧を次ページへ送る | fkey凡例: PF6/PF12=page（L583）／`Already at last page`（L1008） |
| 6 | `PF12` | 出荷数入力画面での押下時 | 明細一覧を前ページへ戻す | fkey凡例: PF6/PF12=page（L583）／`Already at first page`（L1020） |
| 7 | `PF3` | 出荷数入力画面での押下時 | 出荷計上（出庫）の確認へ進む | fkey凡例: PF3=post（L583）／`Confirm shipment (Y) or PF3 to cancel`（L686） |
| **キー・明細入力部** |||||
| 8 | 受注番号 | 入力して `ENTER` 押下時 | 受注ヘッダ・明細を読み込み、引当状態・出荷可否を判定して出荷明細を表示する | 未引当時 `Order is not allocated yet - run OE0030`（L436） |
| 9 | 明細行番号・出荷数量 | 出荷数入力画面で入力して `ENTER` 押下時 | 提案出荷数（残数と在庫の小さい方）を初期値に、引当数／残数を上限として出荷数を確定する | 上限丸め時 `Quantity capped to allocated / outstanding`（L669） |
| 10 | `Confirm shipment (Y) or PF3 to cancel` | 確認時に `Y` 入力時 | 出荷ヘッダ・明細を記帳し（出荷番号は NUMGEN "SHIP" で採番）、在庫を引き落とし、受注明細・受注状態を更新する | 取消時 `Shipment cancelled`（L692/L699） |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | ヘッダ・明細入力での押下時 | 入力内容を確定し次の入力段階（次明細行）へ進む | fkey凡例: ENTER=Next（L146） |
| 2 | `PF3` | 押下時 | 明細入力を終え発注登録の確認へ進む、または業務を終了する | fkey凡例: PF3=End/Finish（L146）／`Enter PO header - PF3 to quit`（L247） |
| 3 | `PF4` | 明細入力での押下時 | 入力中の明細行をクリアする | fkey凡例: PF4=Clear line（L146）／`Line cleared`（L305） |
| **ヘッダ項目部** |||||
| 4 | 仕入先コード | ヘッダ入力で入力して `ENTER` 押下時 | 仕入先マスタを照合し、存在すれば明細入力へ進む | 未存在時 `Supplier not found`（L271）／確定時 `Header OK - enter detail lines`（L289） |
| **明細項目部** |||||
| 5 | 商品コード | 明細入力で入力して `ENTER` 押下時 | 商品マスタで商品を照合する | 未存在時 `Product not found`（L334） |
| 6 | 単価 | 明細入力で未入力のまま `ENTER` 押下時 | 商品の最終原価／標準原価を既定単価として採る（採れなければ入力を要求） | `Unit cost required`（L362） |
| 7 | 倉庫・数量・単価 | 明細入力で確定時 | 倉庫・正の数量・単価を検証し、妥当なら明細に追加する | 追加時 `Line added`（L400） |
| **確定部** |||||
| 8 | `Review totals then confirm` | 確認画面での確定（Y）時 | 発注番号を採番し、発注ヘッダ・明細を登録する | 完了時 `PO saved`（L467）／中止時 `PO discarded`（L417） |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 検索画面での押下時 | 入力した発注番号または仕入先で発注を検索し、該当があれば照会画面へ進む | fkey凡例: ENTER=Search（L152） |
| 2 | `PF3` | 押下時 | 照会を終了しメニューへ戻る | fkey凡例: PF3=End（L152）／`PF3=Back`（L288） |
| 3 | `PF6` | 照会画面（仕入先で参照中）での押下時 | 当該仕入先の次の発注へ表示を送る | メッセージ行 `PF6=Next`（L288）／`No more POs for this supplier`（L306） |
| **キー入力部** |||||
| 4 | 発注番号 | 入力して `ENTER` 押下時 | 入力した発注番号で発注ヘッダを読み、該当があれば照会画面へ進む | 未存在時 `PO number not found`（L209） |
| 5 | 仕入先コード（＋開始日） | 発注番号を空にして入力し `ENTER` 押下時 | 仕入先（＋任意の開始日）の代替キーで発注を辿り、最初の該当を照会画面に表示する | 該当無し時 `No PO found for supplier`（L232） |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した入荷番号で入荷を呼び出し、その明細を仕入明細として次の入力段階へ進める | fkey凡例: ENTER=Next |
| 2 | `PF3` | 押下時 | 仕入入力を終了しメニューへ戻る | fkey凡例: PF3=End |
| **ヘッダ項目部** |||||
| 3 | 入荷番号 | 入力時 | 入力した番号で入荷ヘッダ・明細を照合し、削除・取消・計上済でなく明細があれば仕入対象として取り込む | 該当なしは Receiving not found |
| 4 | 仕入日／税区分 | 入力時 | 仕入日と税区分を受け付け、税額計算を行って仕入金額を確定する（税は TAXCAL で計算） | 案内: Enter purchase date / tax type - PF3 cancel |
| 5 | `Confirm to book the purchase` | 確認入力時 | 承認で仕入番号を採番し、仕入ヘッダ・仕入明細・買掛元帳へ記帳する。否認で仕入を破棄する | 完了: Purchase booked |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力中のヘッダ／返品明細を確定し、次の入力段階へ進める | fkey凡例: ENTER=Next |
| 2 | `PF3` | 押下時 | 返品入力を終了・確定へ移り、明細があれば返品計上、無ければメニューへ戻る | fkey凡例: PF3=End/Finish |
| 3 | `PF4` | 押下時 | 入力中の返品明細を消去する | fkey凡例: PF4=Clear line |
| **ヘッダ項目部** |||||
| 4 | 仕入先コード | 入力時 | 入力した仕入先コードで仕入先マスタを照合し、有効なら返品明細入力へ進む | 削除・不在はエラー |
| **明細項目部** |||||
| 5 | 商品コード／倉庫 | 入力時 | 入力した商品を商品マスタで照合し、倉庫とともに返品対象として取り込む | 削除・不在はエラー |
| 6 | 返品数／単価 | 入力時 | 正の返品数と単価を受け付け、在庫残高を超える場合は警告する | Warning: return qty exceeds on-hand stock |
| **確定部** |||||
| 7 | `Review totals then confirm return` | 確認入力時 | 承認で伝票番号を採番し、仕入伝票（VH-KIND=2）・仕入明細・買掛元帳を作成し、在庫残高を減算して在庫移動履歴を残す。否認で返品を破棄する | 完了: Purchase return posted |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 表示中の明細の入荷数を受理し、次の未入荷明細へ進める | fkey凡例: ENTER=Accept |
| 2 | `PF4` | 押下時 | 表示中の明細を入荷対象から外して次へ進める | fkey凡例: PF4=Skip line |
| 3 | `PF3` | 押下時 | 入荷入力を終了しメニューへ戻る | fkey凡例: PF3=End |
| **ヘッダ項目部** |||||
| 4 | 発注番号 | 入力時 | 入力した発注番号で発注ヘッダ・明細を照合し、削除・取消・全量入荷済でなく未入荷明細があれば各明細を入荷対象として提示する | 未入荷数＝発注数−入荷済 |
| 5 | 入荷日 | 確認時 | 入荷日を確認して入荷計上の対象とする | 案内: Confirm receiving date - PF3 to cancel |
| **明細項目部** |||||
| 6 | 入荷数 | 入力時 | 未入荷数の範囲内で入荷数を受け付ける（0 で当該行を除外） | 負数・超過はエラー |
| **確定部** |||||
| 7 | `Confirm to post receiving` | 確認入力時 | 承認で入荷番号を採番し、入荷ヘッダ・入荷明細を作成、在庫残高を加算して在庫移動履歴を残す。否認で入荷を破棄する | 完了: Receiving posted |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した開始商品／倉庫と並び順で在庫を検索し、該当在庫を 1 件表示する | fkey凡例: ENTER=Search |
| 2 | `PF6` | 押下時 | 次の在庫 1 件を表示する | fkey凡例: PF6=Next |
| 3 | `PF4` | 押下時 | 検索条件を消去して新しい検索キーの入力に戻す | fkey凡例: PF4=New |
| 4 | `PF3` | 押下時 | 在庫照会を終了しメニューへ戻る | fkey凡例: PF3=End |
| **ヘッダ項目部** |||||
| 5 | 開始商品／倉庫・並び順 | 入力時 | 開始商品・倉庫と並び順を受け付け、当該キー以降の在庫残高を在庫・引当・有効数で 1 件ずつ表示する | 該当なしは No stock records found |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した商品・倉庫で在庫残高を読み、現残高を表示して調整入力へ進める | fkey凡例: ENTER=Read |
| 2 | `PF3` | 押下時 | 在庫調整を終了しメニューへ戻る | fkey凡例: PF3=End |
| 3 | `PF4` | 押下時 | 入力した内容を消去する | fkey凡例: PF4=Clear |
| **ヘッダ項目部** |||||
| 4 | 商品コード／倉庫 | 入力時 | 商品を商品マスタ、倉庫を倉庫マスタで照合し、対象の在庫残高を読み込む。無い場合は新規作成扱いとする | No stock record - will be created |
| **明細項目部** |||||
| 5 | 調整数（符号付き）／理由 | 入力時 | 符号付きの調整数と調整理由を受け付ける。調整数 0、調整後マイナス、理由未入力はエラーとする | Result would be negative - not allowed |
| **確定部** |||||
| 6 | `Confirm the new on-hand (Y/N)` | 確認入力時 | `Y` で在庫残高を更新（または新規作成）し、在庫移動履歴を残す。`N` で調整を取り消す | 完了: Adjustment posted |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 表示中の商品の実地棚卸数を保存し、次の商品へ進める | fkey凡例: ENTER=Save/Next |
| 2 | `PF3` | 押下時 | 巡回を打ち切って棚卸計上へ移り、確認後にメニューへ戻る | fkey凡例: PF3=Finish |
| 3 | `PF4` | 押下時 | 表示中の商品を保存せず飛ばして次へ進める | fkey凡例: PF4=Skip |
| **ヘッダ項目部** |||||
| 4 | 倉庫 | 入力時 | 入力した倉庫を倉庫マスタで照合し、当該倉庫の全在庫を棚卸対象として巡回開始する | 在庫なしは No stock records in this warehouse |
| **明細項目部** |||||
| 5 | 実地棚卸数 | 入力時 | 商品ごとに実地棚卸数を受け付け、帳簿在庫と差異（実数−帳簿）を表示する | 上限 500 件 |
| **確定部** |||||
| 6 | `Review counts then confirm posting` | 確認入力時 | 承認で在庫残高を棚卸数へ更新し、在庫移動履歴を残す。否認で計上しない | 完了: Stocktaking posted |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した商品コードの在庫移動履歴を日付順に読み込み、先頭ページを一覧表示する | fkey凡例: ENTER/PF6=Next page |
| 2 | `PF6` | 押下時 | 次ページの在庫移動履歴を続けて表示する | PF6=Next page |
| 3 | `PF4` | 押下時 | 入力内容をクリアし、別の商品コードで照会をやり直す | PF4=New |
| 4 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 5 | 商品コード | 入力時 | 入力した商品コードで商品マスタを照合し、その商品の在庫移動履歴を日付順に検索する | 0 は不可 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した商品・出庫倉庫・入庫倉庫・数量を検証し、問題なければ移動の確認を求める | fkey凡例: ENTER=Read |
| 2 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| 3 | `PF4` | 押下時 | 入力した内容をクリアし、最初から入力し直す | PF4=Clear |
| **ヘッダ項目部** |||||
| 4 | 商品コード | 入力時 | 入力した商品コードで商品マスタを照合し、削除・在庫管理対象を確認する | 0 は不可 |
| 5 | 出庫倉庫・入庫倉庫 | 入力時 | 両倉庫を倉庫マスタで照合し、同一でないこと・出庫倉庫の在庫と引当可能数を確認する | |
| 6 | 移動数量 | 入力時 | 正の値かつ出庫倉庫の引当可能在庫以内であることを確認する | |
| 7 | `Confirm the transfer (Y/N)` | 入力時 | `Y` で移動を確定し、出庫倉庫の在庫を減算・入庫倉庫の在庫を加算し、在庫移動履歴を記録する。`N` で取り消す | 移動番号を採番して記録 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した入金内容を検証し、問題なければ記帳の確認を求め、確定で入金と売掛元帳へ記帳する | fkey凡例: ENTER=Confirm |
| 2 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| 3 | `PF4` | 押下時 | 入力した内容をクリアし、最初から入力し直す | PF4=Clear |
| **ヘッダ項目部** |||||
| 4 | 得意先コード | 入力時 | 入力した得意先コードで得意先マスタを照合し、削除済みでないことを確認する | 未入力は不可 |
| 5 | 入金金額・入金方法 | 入力時 | 金額が正の値であること、入金方法が 1〜4 の範囲であることを確認する | |
| 6 | 銀行コード | 入力時（振込・手形時） | 銀行コードを銀行マスタで照合し、削除済みでないことを確認する | 振込は銀行コード必須 |
| 7 | `Confirm to post the receipt` | 入力時 | `Y` で入金を確定し、入金番号を採番して入金を記録、売掛元帳へ貸方記帳し得意先残高を更新する | |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した得意先コードで売掛元帳を走査し、現残高と年齢表を表示する | fkey凡例: ENTER=Inquire |
| 2 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 3 | 得意先コード | 入力時 | 入力した得意先コードで得意先マスタを照合し、その得意先の売掛元帳を走査する | 0 は不可 |
| 4 | 年齢表 | 走査完了時 | 売掛残高を 0-30／31-60／61-90／90 日超の年齢バケットに区分して表示する | 参照専用 |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した支払内容を検証し、問題なければ記帳の確認を求め、確定で支払と買掛元帳へ記帳する | fkey凡例: ENTER=Confirm |
| 2 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| 3 | `PF4` | 押下時 | 入力した内容をクリアし、最初から入力し直す | PF4=Clear |
| **ヘッダ項目部** |||||
| 4 | 仕入先コード | 入力時 | 入力した仕入先コードで仕入先マスタを照合し、削除済みでないことを確認する | 未入力は不可 |
| 5 | 支払金額・支払方法 | 入力時 | 金額が正の値であること、支払方法が 1〜4 の範囲であることを確認する | |
| 6 | 銀行コード | 入力時（振込時） | 銀行コードを銀行マスタで照合し、削除済みでないことを確認する | 振込は銀行コード必須 |
| 7 | `Confirm to post the payment` | 入力時 | `Y` で支払を確定し、支払番号を採番して支払を記録、買掛元帳へ借方記帳し仕入先残高を更新する | |
...
|---|---|---|---|---|
| **ファンクション部** |||||
| 1 | `ENTER` | 押下時 | 入力した仕入先コードで買掛元帳を走査し、現買掛残高・購入合計・支払合計と直近の元帳明細を表示する | fkey凡例: ENTER=Inquire |
| 2 | `PF3` | 押下時 | 業務を終了しメニューへ戻る | PF3=End |
| **ヘッダ項目部** |||||
| 3 | 仕入先コード | 入力時 | 入力した仕入先コードで仕入先マスタを照合し、その仕入先の買掛元帳を走査する | 0 は不可 |
| 4 | 元帳明細 | 走査完了時 | 現買掛残高・購入合計・支払合計と直近の元帳明細を表示する | 参照専用 |
